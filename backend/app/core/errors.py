...
class CapacityExceededError(ArgusError):
    """A bounded in-process resource (e.g. the capture buffer) is full."""
...
class DependencyNotConfiguredError(ArgusError):
    """A required external component has not been wired up yet (model, DB, index)."""
...
class DependencyUnavailableError(ArgusError):
    """A configured external component is unreachable or failing."""
...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def _integrity(_: Request, exc: IntegrityError) -> JSONResponse:
        # Reached only when a write is not wrapped in `integrity_guard`; a constraint
        # violation is still the caller's problem, not a server fault.
    ...
    async def _database(_: Request, exc: SQLAlchemyError) -> JSONResponse:
        ...
        return JSONResponse(
            ...
            content=error_body(
                ...
                {"driver_error": str(exc.__cause__ or exc)},
            ),
        )
