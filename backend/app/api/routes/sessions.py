"""Class session routes: open, list, close and read the register."""
...
async def list_sessions(
    ...
    # Offset page of sessions, optionally filtered by classroom, status or date.
...
async def close_session(
    ...
    that was never recognised and flips the status to CLOSED, in one transaction
...
async def session_register(
    ...
    after: int | None = Query(default=None, description="Return rows after this roll_no."),
    limit: PageLimit = DEFAULT_PAGE_SIZE,
) -> KeysetPage[AttendanceRecord]:
    # Keyset page of the register for one session.
...
async def session_summary(
    ...
    # Present and absent counts against the current roster size.
